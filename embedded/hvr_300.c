...
MSCB_INFO_CHN code channel[] = {
  1, UNIT_BYTE,            0, 0,           0, "CSR",      &user_data.csr,
  4, UNIT_VOLT,            0, 0, MSCBF_FLOAT, "Vdemand",  &user_data.v_demand,
  4, UNIT_VOLT,            0, 0, MSCBF_FLOAT, "Vmeas",    &user_data.v_meas,
  4, UNIT_AMPERE, PRFX_MICRO, 0, MSCBF_FLOAT, "Imeas",    &user_data.i_meas,
  1, UNIT_COUNT,           0, 0,           0, "TripCnt",  &user_data.trip_cnt,
  0
};

MSCB_INFO_CHN code conf_param[] = {
  2, UNIT_VOLT,            0, 0,           0, "RampUp",   &user_conf.ramp_up,
  2, UNIT_VOLT,            0, 0,           0, "RampDown", &user_conf.ramp_down,
  4, UNIT_VOLT,            0, 0, MSCBF_FLOAT, "Vlimit",   &user_conf.v_limit,
  4, UNIT_AMPERE, PRFX_MICRO, 0, MSCBF_FLOAT, "Ilimit",   &user_conf.i_limit,
  1, UNIT_COUNT,           0, 0,           0, "TripMax",  &user_conf.trip_max,

  /* calibration constants */
  4, UNIT_FACTOR,          0, 0, MSCBF_FLOAT, "ADCgain",  &user_conf.adc_gain,
  4, UNIT_VOLT,            0, 0, MSCBF_FLOAT, "ADCofs",   &user_conf.adc_offset,
  4, UNIT_FACTOR,          0, 0, MSCBF_FLOAT, "DACgain",  &user_conf.dac_gain,
  4, UNIT_VOLT,            0, 0, MSCBF_FLOAT, "DACofs",   &user_conf.dac_offset,
  4, UNIT_FACTOR,          0, 0, MSCBF_FLOAT, "CURgain",  &user_conf.cur_gain,
  4, UNIT_AMPERE, PRFX_MICRO, 0, MSCBF_FLOAT, "CURofs",   &user_conf.cur_offset,

  /* debugging */
  4, UNIT_CELSIUS,         0, 0, MSCBF_FLOAT, "TempC",    &user_conf.temperature,
  4, UNIT_VOLT,            0, 0, MSCBF_FLOAT, "VDAC",     &user_conf.v_dac,
  2, UNIT_WORD,            0, 0,           0, "Debug0",   &user_conf.debug0,
  2, UNIT_WORD,            0, 0,           0, "Debug1",   &user_conf.debug1,
  0
};
...
void user_init(unsigned char init)
{
  ...
  if (init)
    ...
    user_conf.v_limit   = 1200;
    user_conf.i_limit   = 200;
    user_conf.adc_gain  = 1.0;
    user_conf.dac_gain  = 1.0;
    user_conf.cur_gain  = 1.0;
    }

  /* ## temp. calib. const, for testing only */
  user_conf.adc_gain   = 1.00072;
  user_conf.adc_offset = 8.4;
  user_conf.dac_gain   = 1.00000;
  user_conf.dac_offset = -20.8;
  user_conf.cur_gain   = 0.749;
  user_conf.cur_offset = 24.47;
...
void user_write_conf(unsigned char channel) reentrant
{
  ...
  if (channel == 3)
    i_limit_adc = (user_conf.i_limit - user_conf.cur_offset)/user_conf.cur_gain / 
...
unsigned int d;
  ...
  /* check for limit */
  if (value > user_conf.v_limit)
    {
    value = user_conf.v_limit;
  ...
  /* apply correction */
  value = value * user_conf.dac_gain + user_conf.dac_offset;
  if (value < 0)
    value = 0;
...
unsigned char check_current_trip(unsigned int adc)
{
  ...
  user_conf.debug0 = i_limit_adc;
  user_conf.debug1 = adc;
  ...
  if (adc > i_limit_adc)
    ...
    if (user_data.trip_cnt < user_conf.trip_max)
      {
      /* clear trip */
      user_data.csr &= ~CSR_ILIMIT;
...
unsigned long v_average;
  ...
  /* apply calibration */
  hv = hv * user_conf.adc_gain + user_conf.adc_offset;
...
unsigned long c_average;
  ...
  /* calibrate */
  current = current*user_conf.cur_gain + user_conf.cur_offset;
...
unsigned char delta;
  ...
  if ((user_data.csr & CSR_HV_ON) &&
    ...
    if (ramp_up)
      ...
      if (delta)
        {
        v_actual += (float) user_conf.ramp_up * delta / 100.0;
    ...
    if (ramp_down)
      ...
      if (delta)
        {
        v_actual -= (float) user_conf.ramp_down * delta / 100.0;
  ...
  if ((user_data.csr & CSR_HV_ON) && !ramp_up && !ramp_down &&
    ...
    if (user_data.csr & CSR_REGULATION)
      ...
      if (user_data.v_meas > user_data.v_demand + 0.05 ||
        ...
        {
        v_actual = user_data.v_demand;
        user_conf.v_dac += user_data.v_demand - user_data.v_meas;
  
        /* only allow +-3V fine regulation range */
        if (user_conf.v_dac < user_data.v_demand - 3)
          user_conf.v_dac = user_data.v_demand - 3;
  
        if (user_conf.v_dac > user_data.v_demand + 3)
          user_conf.v_dac = user_data.v_demand + 3;
  
        demand_changed = 0;
        dac_write(user_conf.v_dac);
        }
      }
    else
      {
...
unsigned long t_average;
  ...
  if (!ramp_up && !ramp_down)
    ...
    DISABLE_INTERRUPTS;
    user_conf.temperature = temperature;
    ENABLE_INTERRUPTS;
    }
}
