...
INT tr_prestart(INT rn, char *error)
{
   ...
   db_get_value(hDB, 0, "/Ebuilder/Settings/User Field", ebset.user_field, &size,
                TID_STRING, FALSE);
...
INT source_booking(INT nfrag)
{
   ...
   for (i = 0; i < nfrag; i++) {
      ...
      if (ebch[i].set.emask) {
         ...
         status1 = bm_open_buffer(ebch[i].set.buffer, EVENT_BUFFER_SIZE, &(ebch[i].hBuf));
         ...
         status2 =
             bm_request_event(ebch[i].hBuf, ebch[i].set.event_id,
                              ebch[i].set.trigger_mask, GET_ALL, &ebch[i].req_id, NULL);
         if (debug)
         ...
         case BM_SUCCESS:      /* event received */
            ...
            if (debug1) {
               printf("SUCC: ch:%d ser:%d Dest_emask:%d cdemask:%x emask:%x sz:%d\n", i,
                      ebch[i].serial, ebset.emask, cdemask, ebch[i].set.emask, size);
            }
            break;
      ...
      if ((actual_millitime = ss_millitime()) - last_time > 1000) {
         ...
         for (j = 0; j < nfragment; j++) {
            ...
            if ((actual_millitime > start_time) && ebch[j].stat.events_sent) {
               ...
               db_set_record(hDB, ebch[j].hStat, &(ebch[j].stat)
