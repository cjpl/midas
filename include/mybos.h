...
typedef struct {
  INT  *top_ptr;         /* top ring_buffer data pointer */
  INT  *bot_ptr;         /* bottom ring_buffer data pointer */
  INT  *rd_cur_ptr;      /* current read data pointer (from buffer to tape) */
  INT  *wr_cur_ptr;      /* current write (from event to buffer) */
  INT  *blk_end_ptr;     /* end of Ybos data block pointer */
  INT   event_offset;    /* offset to first logical record in bytes */
  INT   cur_block;       /* current YBOS block number */
} YBOS_INFO;

/* YBOS Physical record header */
typedef struct {
  INT   block_size;        /* LPR Length of Physical Record (exclusive) */
  INT   header_length;     /* LPH Length of Physical Header (inclusive) */
  INT   block_num;         /* NPR Physical Record Number (start 0) */
  INT   offset;            /* LRD Offset to 1st Logical Record (=LPH for 1st logical record */
...
INT   EXPRT ybos_swap_event (DWORD * pevt);
INT   EXPRT ybos_open_datafile (char * filename);
INT   EXPRT ybos_close_datafile (void);
INT   EXPRT ybos_skip_physrec (INT bl);
INT   EXPRT ybos_physhead_get (void);
INT   EXPRT ybos_physrec_get (void);
INT   EXPRT ybos_physhead_display (void);
INT   EXPRT ybos_physrec_display (void);
INT   EXPRT ybos_evtlen_display (void);
INT   EXPRT ybos_event_get (INT bl, DWORD * prevent);
...
void  EXPRT ybkclose(DWORD *pbktop ,YBOS_BANK_HEADER **pbkh ,DWORD **pbkdat);
void  EXPRT ybkopen (DWORD *pbktop, char *bname, DWORD bt, YBOS_BANK_HEADER **pbkh, DWORD **pbkdat);
INT   EXPRT ybklist(DWORD *pevt , char *bklist);
INT   EXPRT ybkfind (DWORD *pevt , char * bkname, DWORD * bklength, DWORD * bktype, DWORD **pbkdata);
INT   EXPRT ybkiterate (DWORD * pevt , YBOS_BANK_HEADER ** pybkh, void ** pbkdata);
