      ...
      if (actual_time != last_time_display)
        ...
        for (i=0 ; equipment[i].name[0] ; i++)
          ...
          eq->stats.events_per_sec =
            (DWORD) (eq->events_sent / ((actual_time - last_time_display)/1000.0));
...
usage:
  ...
  status = cm_connect_experiment(host_name, exp_name, fal_name, NULL);
  if (status != CM_SUCCESS)
    {
  ...
  if (status == CM_SUCCESS)
    ...
    cm_disconnect_experiment();
    cm_connect_experiment(host_name, exp_name, fal_name, NULL);
    }
