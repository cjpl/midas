          ...
          if (status != BM_SUCCESS && status != BM_CREATED)
            {
              cm_msg(MERROR, "register_equipment", 
                     "Cannot open event buffer. Try to reduce EVENT_BUFFER_SIZE in your FE code\
...
void send_all_periodic_events(INT transition)
{
  ...
  /* send_all_periodic_events */
  /* flush ALL event sitting in dm either do it now or semaphore it
     but in this last case wait for completion (there is a timeout) 
     No abort in case of timeout (void ??) */
#ifdef DM_DEBUG
  printf("All periodic flush\n");
#endif
...
\********************************************************************/
{
  ...
  /* only if the equipment is EQ_INTERRUPT */
  if (interrupt_eq)
    {
...
#ifdef OS_VXWORKS
  rpc_set_opt_tcp_size(1<<10);
#ifdef PPCxxx
  rpc_set_opt_tcp_size(NET_TCP_SIZE);
#endif
#endif
  /* scheduler */
  /* PAA cannot have consecutive dm_pointer_get() due to +sizeof(INT)
     to be checked */
  /*----------------- MAIN equipment loop ------------------------------*/
  do
    {
      ...
      for (index=0 ; ; index++)
              ...
              if (actual_millitime - eq->last_called >= (DWORD) eq_info->period)
                {
                  ...
                  /* get pointer for upcoming event. dm_area_send taken care inside */
                  /* return value should be valid pointer. if NULL BIG error ==> abort */
                  if ((pevent = dm_pointer_get()) == NULL)
                    {
                      ...
                      if (eq->buffer_handle)
                        {
                          ...
                          dm_pointer_increment(eq->buffer_handle, 
                                               pevent->data_size + sizeof(EVENT_HEADER));
                          /* in PERIODIC or SLOW so flush right now */
#ifdef DM_DEBUG
                          printf("Periodic flush \n");
#endif
                          if ((status = dm_area_flush()) != CM_SUCCESS)
                            {
                              cm_msg(MERROR,"scheduler","dm_area_flush failed %i",status);
                              goto net_error;
                            }
#else
                          rpc_flush_event();
              ...
              while ((source = poll_event(eq_info->source, eq->poll_count, FALSE)) > 0)
                {
                  ...
                  /* PAA overwritten by next statement but valid for extraction in 
                     user readout function for private use (SR feature) */
                  *(INT *) (pevent+1) = source;
          ...
          if (eq_info->eq_type == EQ_INTERRUPT)
            {
              ...
              /* printf("Event:%i %i \r",eq->serial_number,eq->stats.events_per_sec );
                 fflush (stdout); */

              /* if rate is low force flush to guarantee data at server side */
              if ((actual_millitime - dm_buffer_time_get()) > 2000) 
                {
                  interrupt_enable(FALSE);
                  if ((status = dm_area_flush()) != CM_SUCCESS)
                    cm_msg(MERROR,"EQ_INTERRUPT","dm_area_flush: %i",status);
                  interrupt_enable(TRUE);
                 }
            } /* end of interrupt equipments */
        ...
        } /* end of loop over all equipments */
...
#endif
{
...
#ifdef OS_VXWORKS
  /* memory increased for VxWorks */
  if (MAX_EVENT_SIZE > memFindMax())
    {
      cm_msg(MERROR,"mainFE","Not enough mem space for event size");
      return 0;
    }
  /* reserve memfree space less 20% */
  eb_size = 0.8 * memFindMax();
  ...
  printf("max event size         :     %i\n",MAX_EVENT_SIZE);
  printf("# of events per buffer :     %i\n",eb_size/MAX_EVENT_SIZE);
#endif
#ifdef OS_MSDOS
...
/* book buffer space */
  status = dm_buffer_create(eb_size);
  if (status != CM_SUCCESS)
    {
      printf("Not enough memory. Decrease event_buffer_size in your FE code\n");
      return 1;
    }
