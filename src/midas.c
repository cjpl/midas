...
INT cm_set_watchdog_params(BOOL call_watchdog, DWORD timeout)
{
   ...
   if (rpc_get_server_option(RPC_OSERVER_TYPE) != ST_REMOTE) {
      ...
      for (i = _buffer_entries; i > 0; i--) {
         ...
         index = bm_validate_client_index(&_buffer[i - 1]);
...
INT cm_transition(INT transition, INT run_number, char *perror, INT strsize, INT async_flag, INT debug_flag)
{
   ...
   if (debug_flag == 2)
      cm_msg(MDEBUG, "cm_transition", "cm_transition: ---- Transition %s finished ----", trname);
...
INT bm_close_buffer(INT buffer_handle)
{
...
#ifdef LOCAL_ROUTINES
   {
      ...
      index = bm_validate_client_index(&_buffer[buffer_handle - 1]);
...
#ifdef LOCAL_ROUTINES
   {
      ...
      for (i = 0; i < _buffer_entries; i++)
         if (_buffer[i].attached) {
            ...
            pheader = _buffer[i].buffer_header;
            pbclient = pheader->client;
            pbclient[bm_validate_client_index(&_buffer[i])].last_activity = ss_millitime();
...
INT bm_add_event_request(INT buffer_handle, short int event_id,
                         short int trigger_mask,
                         INT sampling_type,
                         void (*func) (HNDLE, HNDLE, EVENT_HEADER *, void *), INT request_id)
/********************************************************************\
...
\********************************************************************/
{
...
#ifdef LOCAL_ROUTINES
   {
      ...
      pclient = &(_buffer[buffer_handle - 1].buffer_header->
                  client[bm_validate_client_index(&_buffer[buffer_handle - 1])]);
                     ...
                     void (*func) (HNDLE, HNDLE, EVENT_HEADER *, void *))
{
   ...
   *request_id = index;
   ...
   status = bm_add_event_request(buffer_handle, event_id, trigger_mask, sampling_type, func, index);
...
static int bm_copy_from_cache(BUFFER * pbuf, void *destination, int max_size,
                              int *buf_size, int convert_flags)
{
   int status;
   EVENT_HEADER *pevent;
   int size;
         ...
         if (pc->pid) {
            if (pc->read_pointer == pheader->read_pointer) {
               ...
               if (blocking) {
                  ...
                  int increment = sizeof(EVENT_HEADER) +
...
INT bm_send_event(INT buffer_handle, void *source, INT buf_size, INT async_flag)
{
...
#ifdef LOCAL_ROUTINES
   {
      ...
      if (total_size >= pheader->size) {
         cm_msg(MERROR, "bm_send_event",
                "total event size (%d) larger than buffer size (%d)", total_size, pheader->size);
         return BM_NO_MEMORY;
      }
...
\********************************************************************/
{
...
#ifdef LOCAL_ROUTINES
   {
      ...
      for (i = 0; i < _buffer_entries; i++) {
         ...
         pheader = _buffer[i].buffer_header;
         pclient = pheader->client + bm_validate_client_index(&_buffer[i]);
...
\********************************************************************/
{
   ...
   struct sockaddr_in bind_addr;
   INT sock;
   INT remote_hw_type, hw_type, remote_revision;
   char str[200];
   ...
   sprintf(str, "%d %d %s %s", hw_type, cm_get_revision(), local_prog_name, local_host_name);
   ...
   remote_hw_type = remote_revision = 0;
   sscanf(str, "%d %d", &remote_hw_type, &remote_revision);
   _client_connection[index].remote_hw_type = remote_hw_type;
   _client_connection[index].send_sock = sock;

   /* print warning if revision doesn't agree */
   if (remote_revision != cm_get_revision()) {
      sprintf(str, "remote MIDAS revision %d differs from local revision %d", 
              remote_revision, cm_get_revision());
...
\********************************************************************/
{
   ...
   INT remote_hw_type, hw_type, revision;
   unsigned int size;
   char str[200];
   char local_prog_name[NAME_LENGTH];
   struct hostent *phe;
...
/*    cm_msg(MERROR, "rpc_server_connect", "cannot connect"); message should be displayed by application */
   ...
   if (exp_name[0] == 0)
      sprintf(str, "C %d %d %d %d Default", listen_port1, listen_port2, listen_port3, cm_get_revision());
   else
      sprintf(str, "C %d %d %d %d %s", listen_port1, listen_port2, listen_port3, cm_get_revision(), exp_name);
   ...
   status = revision = 0;
   sscanf(str, "%d %d", &status, &revision);
   ...
   if (revision != cm_get_revision()) {
      sprintf(str, "remote MIDAS revision %d differs from local revision %d", revision, cm_get_revision());
      ...
      if (write_ptr - read_ptr >= (INT) sizeof(NET_COMMAND_HEADER) - copied) {
         if (param_size == -1) {
            ...
            if (_server_acception[index].convert_flags)
               rpc_convert_single(&param_size, TID_DWORD, 0, _server_acception[index].convert_flags);
         }
...
/********************************************************************/
INT recv_event_server(INT index, char *buffer, DWORD buffer_size, INT flags, INT * remaining)
/********************************************************************\
         ...
         if (convert_flags) {
            if (flags & RPC_VARARRAY)
               ...
               rpc_convert_data(in_param_ptr, tid, flags,
                                rpc_list[index].param[i].n * tid_size[tid], convert_flags);
         }
         ...
         if (convert_flags) {
            if (flags & RPC_VARARRAY)
               rpc_convert_data(out_param_ptr, tid,
                                rpc_list[index].param[i].flags | RPC_OUTGOING, param_size, convert_flags);
            else
               rpc_convert_data(out_param_ptr, tid,
...
\********************************************************************/
{
   ...
   if (i > 0) {
      ...
      case 'C':
         ...
         port1 = port2 = revision = 0;
         sscanf(net_buffer + 2, "%d %d %d %d", &port1, &port2, &port3, &revision);
         strcpy(callback.experiment,
                strchr(strchr(strchr(strchr(net_buffer + 2, ' ') + 1, ' ') + 1, ' ') + 1, ' ') + 1);

         /* print warning if version patch level doesn't agree */
         if (revision != cm_get_revision()) {
            sprintf(str, "client MIDAS revision %d differs from local revision %d", 
                    revision, cm_get_revision());
         ...
         if (rpc_get_server_option(RPC_OSERVER_TYPE) == ST_MPROCESS) {
            ...
            sprintf(str, "1 %d", cm_get_revision());     /* 1 means ok */
            send(sock, str, strlen(str) + 1, 0);
            closesocket(sock);
         } else {
            sprintf(str, "1 %d", cm_get_revision());     /* 1 means ok */
...
*/
   ...
   hw_type = rpc_get_option(0, RPC_OHW_TYPE);
   sprintf(str, "%d %d", hw_type, cm_get_revision());
...
\********************************************************************/
{
   ...
   if (sock == _server_acception[index].recv_sock) {
      do {
         ...
         if (_server_acception[index].remote_hw_type == DR_ASCII)
            status = rpc_execute_ascii(_server_acception[index].recv_sock, _net_recv_buffer);
         else
            status = rpc_execute(_server_acception[index].recv_sock,
                                 _net_recv_buffer, _server_acception[index].convert_flags);
...
/* printf("Send watchdog message to %s on %s\n",
         ...
         i = send_tcp(_server_acception[index].send_sock, (char *) &nc, sizeof(NET_COMMAND_HEADER), 0);
 ...
 exit:
   ...
   cm_msg(MINFO, "rpc_check_channels", "client [%s]%s failed watchdog test after %d sec",
          _server_acception[index].host_name,
          _server_acception[index].prog_name, _server_acception[index].watchdog_timeout / 1000);
...
INT hs_read(DWORD event_id, DWORD start_time, DWORD end_time,
            ...
            DWORD * time_buffer, DWORD * tbsize, void *data_buffer, DWORD * dbsize, DWORD * type, DWORD * n)
/********************************************************************\
