...
/*------------------------------------------------------------------*/
/** @name cm_set_msg_print()
\begin{description}
\item[Description:] Set message masks. When a message is generated by calling
cm_msg, it can got to two destinatinons. First a user defined callback routine
...
/** @name cm_msg()
\begin{description}
\item[Description:] This routine can be called whenever an internal error occurs
...
@param message_type See \Ref{Message Macros}.
...
/** @name cm_msg1()
\begin{description}
\item[Description:] This routine is similar to \Ref{cm_msg()}.
It differs from cm_msg() only by the logging destination being a file
given through the argument list i.e:{\bf facility}.
\item[Remarks:] Do not add the "\\n" escape carriage control at the end of the
formated line as it is already added by the client on the receiving side.
...
\end{verbatim}
\end{description}
@memo Redirect messages to a prive log file.
@param message_type See \Ref{Message Macros}.
@param filename Name of source file where error occured
@param line Line number where error occured
...
/** @name cm_connect_experiment()
    ...
    \item[Remarks:] All MIDAS applications should evaluate the MIDAS_SERVER_HOST
...
\end{verbatim}
    ...
    @return CM_SUCCESS, CM_UNDEF_EXP, CM_SET_ERROR, RPC_NET_ERROR, \\
...
\end{verbatim}
    ...
    @param transition Transition to register for. Can be TR_PRESTART, TR_START,
    TR_POSTSTART, TR_PRSTOP, TR_STOP, TR_POSTSTOP, TR_PAUSE or TR_RESUME.
...
/** @name bm_request_event()
    ...
    @return BM_SUCCESS, BM_INVALID_HANDLE\\
    BM_NO_MEMORY  too many requests. The value
    MAX_EVENT_REQUESTS in midas.h should be
    increased.
*/
...
/** @name bm_delete_request()
    ...
    When a buffer is closed via bm_close_buffer(), all
...
/** @name bm_send_event()
    ...
    *(event+sizeof(EVENT_HEADER)) = <...>
    ...
    @return BM_SUCCESS, BM_INVALID_HANDLE, BM_INVALID_PARAM\\
...
/** @name bm_receive_event()
    ...
    command line prompt.
    \item[Example:] The according bm_request_event() call contains NULL as the
...
/** @name bk_init32()
    ...
    bank size (> 32KBytes)
    \item[Remarks:] Before banks can be created in an event, bk_init32()
    has to be called first.
    \end{description}
...
/** @name bk_size()
    ...
    bk_size() plus the size of the event header (sizeof(EVENT_HEADER)).
    \end{description}
...
/** @name bk_swap()
    ...
    \item[Remarks:] An event contains a flag which is set by bk_init() to identify
