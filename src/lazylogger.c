...
/*------------------------------------------------------------------*/
INT lazy_select_purge(HNDLE hKey, INT channel, LAZY_INFO * pLall
            , char * fmt, char * dir, char * fpufile, INT * run)
/********************************************************************\
...
\********************************************************************/
{
  ...
  cpy_loop_time = -2000;
  {
    ...
    if (msg_flag) cm_msg(MTALK,"Lazy",str);
  ...
  if (lazy_file_exists(lazy.dir, lazyst.backfile))
  {
    ...
    if ((lazy.capacity < (lazyst.cur_dev_size + lazyst.file_size)) || eot_reached)
    {
      ...
      if(dev_type == LOG_TYPE_TAPE)
      {
        ...
        if (channel < 0) {
          cm_msg(MERROR,"Lazy", "Cannot rewind tape %s - %d - %d"
            , outffile, channel, status);
          return NOTHING_TODO;
        }
        //else 
...
usage:
  ...
  { /* reconnect to experiment with proper name */
    ...
    status = cm_connect_experiment1(host_name, expt_name, str, 0
      , DEFAULT_ODB_SIZE, WATCHDOG_TIMEOUT);
  }
  if (status != CM_SUCCESS)
    goto error;
    ...
    (db_find_key(hDB,0,"/Logger/Data dir", &hKey) == DB_SUCCESS))
  {
    ...
    db_set_value(hDB, lazyinfo[channel].hKey, "Settings/Data dir", lazy.dir
      , size, 1, TID_STRING);
  }
