/********************************************************************\
  ...
  $Log$
  Revision 1.80  2005/01/07 09:29:05  midas
  Version 1.7.a
  ...
  Revision 1.77  2004/12/08 10:42:18  midas
  Version 1.7.8

  Revision 1.76  2004/10/29 12:46:14  midas
  Version 1.7.7
...
#define MSCB_LIBRARY_VERSION   "1.7.a"
...
\********************************************************************/
{
   ...
   */
   ...
   for (i=0 ; i<=size/58 ; i++) {
      ...
      memcpy(buf+3, (char *)data+i*58, n);
      crc = crc8(buf, 3 + n);
      buf[3 + n] = crc;

      mscb_out(fd, buf, n+4, 0);

      /* read acknowledge */
      n = mscb_in(fd, ack, 2, 10000);
      if (n < 2) {
         mscb_release(fd);
         return MSCB_TIMEOUT;
      }
