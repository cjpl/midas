...
struct hostent       *phe;
  ...
  *pinfo = info;
  ...
  if (info->sock == -1)
    {
    cm_msg(MERROR, "bb_psi", "cannot create socket");
    return FE_ERR_HW;
    }
  ...
  if (status < 0)
    {
    cm_msg(MERROR, "bb_psi", "cannot bind");
    return RPC_NET_ERROR;
    }
  ...
  bind_addr.sin_family      = AF_INET;
  bind_addr.sin_addr.s_addr = 0;
  bind_addr.sin_port        = htons((short) info->bb_psi_settings.port);

#ifdef OS_VXWORKS
  {
  INT host_addr;

  host_addr = hostGetByName(info->bb_psi_settings.frontend_pc);
  memcpy((char *)&(bind_addr.sin_addr), &host_addr, 4);
  }
#else
  phe = gethostbyname(info->bb_psi_settings.frontend_pc);
  if (phe == NULL)
    {
    cm_msg(MERROR, "bb_psi", "cannot get host name");
    return RPC_NET_ERROR;
    }
  ...
  if (status != 0)
    {
    cm_msg(MERROR, "bb_psi", "cannot connect to %s", info->bb_psi_settings.frontend_pc);
    return FE_ERR_HW;
    }
...
INT  status;
  ...
  sprintf(str, "WCAM %d %d 16 %d", info->bb_psi_settings.ncom, info->bb_psi_settings.acom,
          ((int)value) == 1 ? info->bb_psi_settings.com_open : info->bb_psi_settings.com_close);

  status = send(info->sock, str, strlen(str)+1, 0);
  if (status < 0)
    {
    cm_msg(MERROR, "bb_psi_get", "cannot read data from %s", info->bb_psi_settings.frontend_pc);
    return FE_ERR_HW;
    }
...
static DWORD last_update;
  ...
  if (ss_time() - last_update > 60 ||
    ...
    sprintf(str, "RCAM %d %d 0", info->bb_psi_settings.nstat, info->bb_psi_settings.astat);
    status = send(info->sock, str, strlen(str)+1, 0);
    if (status <= 0)
      {
