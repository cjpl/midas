...
HNDLE        hDB;
  ...
  for (i=0 ; i<hv_info->num_channels ; i++)
    ...
    if ( abs(hv_info->measured[i] - hv_info->measured_mirror[i]) > hv_info->update_threshold[i])
      changed = TRUE;
  ...
  for (i=0 ; i<hv_info->num_channels ; i++)
    ...
    if ( abs(hv_info->current[i] - hv_info->current_mirror[i]) > hv_info->update_threshold_current[i])
      changed = TRUE;
...
HV_INFO *hv_info;
  ...
  db_open_record(hDB, hKey, hv_info->voltage_limit, sizeof(float) * hv_info->num_channels, 
                 MODE_READ, NULL, NULL);
  ...
  db_open_record(hDB, hKey, hv_info->current_limit, sizeof(float) * hv_info->num_channels, 
  ...
  db_open_record(hDB, hv_info->hKeyDemand, hv_info->demand, 
                 hv_info->num_channels*sizeof(float), MODE_READ, hv_demand, pequipment);
  ...
  memcpy(hv_info->measured_mirror, hv_info->measured, hv_info->num_channels * sizeof(float));
  ...
  memcpy(hv_info->current_mirror, hv_info->current, hv_info->num_channels * sizeof(float));
