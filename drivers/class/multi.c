...
HNDLE        hDB;
  ...
  if (channel == -1)
    ...
    m_info->var_input[channel] = m_info->var_input[channel] * m_info->factor_input[channel] -
                                 m_info->offset_input[channel];
    }

  /* check if significant change since last ODB update */
  for (i=0 ; i<m_info->num_channels_input ; i++)
    if ( abs(m_info->var_input[i] - m_info->input_mirror[i]) > m_info->update_threshold[i])
      break;
  ...
  if (i < m_info->num_channels_input || actual_time - m_info->last_update > 60000)
    ...
    db_set_data(hDB, m_info->hKeyInput, m_info->var_input, m_info->num_channels_input*sizeof(float), 
...
EQUIPMENT  *pequipment;
  ...
  for (i=0 ; i<m_info->num_channels_output ; i++)
    ...
    if (m_info->var_output[i] != m_info->output_mirror[i])
      ...
      DRIVER_OUTPUT(i)(CMD_SET, m_info->dd_info_output[i], i-m_info->channel_offset_output[i], 
...
MULTI_INFO *m_info;
  ...
  db_open_record(hDB, hKey, m_info->update_threshold, m_info->num_channels_input*sizeof(float), 
                 MODE_READ, NULL, NULL);
  ...
  db_open_record(hDB, hKey, m_info->offset_input, m_info->num_channels_input*sizeof(float), 
                 MODE_READ, NULL, NULL);
  ...
  for (i=0 ; pequipment->driver[i].name[0] ; i++)
    ...
    if (status != DB_SUCCESS)
      ...
      if (status != DB_SUCCESS)
        {
        cm_msg(MERROR, "multi_init", "Cannot create %s entry in online database", str);
        free_mem(m_info);
        return FE_ERR_ODB;
        }
      }
