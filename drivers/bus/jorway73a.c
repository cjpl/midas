...
/* static */ int handle_scsi_cmd( int branch,          /* CAMAC branch */
                             ...
                             )
  {
      ...
      fprintf(stderr,"handle_scsi_cmd: branch=%d, idx=%d, fd=%d, cmd_len=%d, in_size=%d, ibuff=0x%x, out_size=%d, o_buff=0x%x\n",
...
{ return(1); }
int sjy_getsem()
{ return(0); } /* get the sjy semaphore */
int sjy_putsem()
{ return(0); } /* release the sjy semaphore */
#endif
     ...
     /* Essentially copied from before */
{
  ...
  switch (status ) {
  case 0: 
    sjy_controller[idx].errn=ENODEV; break;  /* X=1,Q=0; see get_qx */
  case 4:
    sjy_controller[idx].errn=0; break;       /* X=1,Q=1; see get_qx */
  case 2: 
    switch (addsense) {
    case SJY_NO_Q:
      sjy_controller[idx].errn=ENODEV;break; /* X=1,Q=0 */
    case SJY_NO_X:
      sjy_controller[idx].errn=EFAULT;break; /* X=0 but SCSI ok */
    default:
      sjy_controller[idx].errn=EIO;break; /* SCSI failed, too. */
...
#endif
    sjy_controller[idx].errn = 0; return; }
  else if (target_status!=2) { /* Improper status */
    sjy_controller[idx].errn = EIO;
    fprintf(stderr,"Invalid SCSI status byte: 0x%02x\n",target_status); }
  else { /* Status = 2, check condition */
#if GDEBUG
    fprintf(stderr,"target_status=0x%02x,sense_key=0x%02x,add_sense=0x%02x\n",
