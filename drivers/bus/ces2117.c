...
{
  *d = (unsigned short) *((unsigned long *)(0xC00000|((((f<<5)|n)<<4)|a)<<2)) & 0xFFFFFF;
}
...
int  i;
  ...
  wait; *cio_ADRS = (n <= 8 || n > 16) ? ZCIO_PAPM : ZCIO_PBPM; 
  wait; lam_mask = *cio_ADRS;


  if (l) { actLM |= lam->lm;} else { actLM &= ~lam->lm; };  
  ...
  wait; *lam->cc = lam->pm; wait; *lam->cc = actLM; 
  wait; *lam->cc = lam->cs; wait; *lam->cc = ZCIO_CS_CLIPIUS; 
  wait;
  intUnlock(i);
...
{ 
  ...
  if (lam->cc != NULL) {                /* enable/disable CIO-interrupt   */
    actLM = *lam->cc;
    wait; *lam->cc = lam->pm; wait; actLM = *lam->cc;  
    if (l) { actLM |= lam->lm;} else { actLM &= ~lam->lm; };  
  ...
  if (l) return ((((*(int *)(lam->ext|(26<<VCCshiftF))) & VCCmaskX) != 0) ? 0 : -1);
         return ((((*(int *)(lam->ext|(24<<VCCshiftF))) & VCCmaskX) != 0) ? 0 : -1);
}
