...
CA_INFO   *info;
  ...
  *pinfo = info;
  ...
  for (i=0 ; i<channels ; i++)
    ...
    if (!(status & CA_M_SUCCESS))
      cm_msg(MERROR, "epics_ca_init", "cannot find tag %s", info->channel_names+CHN_NAME_LENGTH*i);
    }
  ...
  for (i=0 ; i<channels ; i++)
    ...
    if (!(status & CA_M_SUCCESS))
      cm_msg(MERROR, "epics_ca_init", "cannot add event to channel %s", info->channel_names+CHN_NAME_LENGTH*i);
    }
