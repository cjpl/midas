...
int cwrite ( CAMAC_t *camac, unsigned char n, unsigned char a,
	     unsigned char f, cdata_t d );
int cexecute( CAMAC_t *camac, unsigned char n,
	      unsigned char a, unsigned char f );
int cblkstart( CAMAC_t *camac, int type, int bits , unsigned long nb);
void cblkgo( CAMAC_t *camac, unsigned char n, unsigned char a,
	     unsigned char f );
...
INLINE void cam24o(const int c, const int n, const int a, const int f, DWORD d)
{
  ...
  cselect(c);
  c_t.n = n; c_t.a = a; c_t.f = f; c_t.d = d;
  if ((stat = ioctl( fd, CAMAC_EXEC, &c_t )) != 0) {
    perror( fname );
  }
}
...
int cblock( int n, int a, int f, int mode, int bits,
