...
CMKey::CMKey(CMKey * pRootKey, CString strKeyName)
{
   ...
   m_bValid = (db_find_key(m_hDB, pRootKey->GetHkey(), strKeyName.GetBuffer(256), &m_hKey) == DB_SUCCESS);
   if (m_bValid)
      db_get_key(m_hDB, m_hKey, &m_Key);
   else {
      CString msg("Cannot find " + strKeyName + " entry in online database");
...
INT CMKey::SaveXML(CString strFileName)
{
   return db_save_xml(m_hDB, m_hKey, (char *) LPCTSTR(strFileName));
}
